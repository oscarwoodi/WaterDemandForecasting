...
class ARIMAXModel:
    ...
    def roll_predict(self, no_test, no_batch): 
        ...
        for i in np.arange(0, no_test/no_batch): 
            ...
            params, seasonal_params = self.find_best_parameters(epoch_train, epoch_train_diurnal)
            fit_models = self.train_model(params, seasonal_params, epoch_train, epoch_train_diurnal)  
    ...
    def find_best_parameters(self, train, train_diurnal):
        ...
        sarima_params = {dma: () for dma in self.dmas}

        for dma in self.dmas: 

            arima_m = 24  # daily seasonality

            arima_d = ndiffs(train[dma]-train_diurnal[dma],max_d = 12)
            arima_D = nsdiffs(train[dma]-train_diurnal[dma], m = arima_m,max_D = 5)

            arima_model = auto_arima(train[dma]-train_diurnal[dma], 
                                    d = arima_d, D = arima_D, m = arima_m,
            ...
            sarima_params[dma] = arima_model.get_params()['seasonal_order']
    ...
    def train_model(self, arima_params, seasonal_params, train, train_diurnal):
        """
        ...
        for dma in self.dmas:
            ...
            model_arimax = SARIMAX(train[dma]-train_diurnal[dma],
    ...
    def predict(self, models, train, test, test_diurnal, window):
        ...
        for dma in self.dmas: 
            ...
            #Test Prediction
            forecast_arimax = models[dma].get_prediction(len(train),len(train)+window-1)
            testPredict_arimax = forecast_arimax.predicted_mean + test_diurnal[dma]
            testPredict_arimax_ci = forecast_arimax.conf_int(alpha=0.1)

            # extract forecast
            results[dma] = testPredict_arimax
            results['lower_'+dma] = testPredict_arimax_ci.iloc[:,0] + test_diurnal[dma]
            results['upper_'+dma] = testPredict_arimax_ci.iloc[:,1] + test_diurnal[dma]
