...
class TBATSModel:
    """
    TBATS model on demand - diurnal and daily & weekly seasonality.
    """
    def __init__(self):
        self.model = None
    ...
    def train_model(self, seasonality):
        ...
        for dma in self.dmas:
            ...
            model_tbats = TBATS(
                ...
                use_box_cox=False)
            models[dma]=model_tbats.fit(self.train[dma]-self.train_diurnal[dma])
            print(model_tbats.get_fitted_params())
            # include exogeneous? 
    ...
    def predict(self, models):
        ...
        for dma in self.dmas: 
            ...
            #Test Prediction
            testPredict_arima = models[dma].predict(fh)+self.test_diurnal[dma]
            testPredict_arima_ci = models[dma].predict_interval(fh,coverage=0.95)
            testPredict_arima_ci.iloc[:,0] = testPredict_arima_ci[dma][0.95]['lower'].iloc[:,0]+self.test_diurnal[dma]
            testPredict_arima_ci.iloc[:,1] = testPredict_arima_ci[dma][0.95]['upper'].iloc[:,0]+self.test_diurnal[dma]
