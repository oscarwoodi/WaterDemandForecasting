...
class ARIMAXModel:
    ...
    def import_data(self, train_window, filepath_demand, filepath_weather, filepath_diurnal):
        ...
        self.diurnal_inflow_data = self.diurnal_inflow_data[~self.diurnal_inflow_data.index.duplicated(keep='first')]
    ...
    def find_best_parameters(self):
        ...
        for dma in self.dmas: 
            arima_d = ndiffs(self.train[dma],max_d = 12)
            arima_model = auto_arima(self.train[dma], d = arima_d, 
                                    exogenous = self.train_diurnal[dma],
    ...
    def train_model(self, arima_params):
        ...
        for dma in self.dmas:
            ...
            model_arimax = ARIMA(self.train[dma],
                                order = arima_params[dma], 
                                exog = self.train_diurnal[dma], 
                                freq="H", 
                                )
    ...
    def predict(self, models):
        ...
        for dma in self.dmas: 
            ...
            #Train prediction
            trainPredict_arimax = (models[dma].predict(0,len(self.train)-1)).fillna(0)

            #Test Prediction
            forecast_arimax = models[dma].get_prediction(len(self.train),len(self.inflow_df)-1,exog=self.test_diurnal[dma])
            testPredict_arimax = forecast_arimax.predicted_mean
            testPredict_arimax_ci = forecast_arimax.conf_int(alpha=0.1)

            # extract forecast
            self.results[dma] = testPredict_arimax
            self.results['lower_'+dma] = testPredict_arimax_ci.iloc[:,0]
            self.results['upper_'+dma] = testPredict_arimax_ci.iloc[:,1]
